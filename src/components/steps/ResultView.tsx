    ...
    <>
      ...
      {isWhatsApp && (
        <div className="flex gap-1 mb-3.5 bg-ink-card rounded-card-lg p-1 border border-ink-border">
          <button
            ...
            className={`flex-1 py-2 text-[11px] font-semibold rounded-card transition-all ${
              activeTab === 'preview'
          ...
          <button
            ...
            className={`flex-1 py-2 text-[11px] font-semibold rounded-card transition-all ${
              activeTab === 'edit'
                ? 'bg-bronze/20 text-bronze shadow-md'
                : 'text-tx-dim hover:text-tx'
            }`}
          >
            Edit &amp; Export
      ...
      {isWhatsApp && activeTab === 'preview' && (
        <>
          ...
          {/* Source */}
          {result.sourceUrl && (
            <div className="bg-ink-card border border-ink-border rounded-card px-3.5 py-2 flex justify-between items-center mt-2">
      ...
      {/* ═══ EDIT & EXPORT TAB (also the default for non-WhatsApp) ═══ */}
      {(!isWhatsApp || activeTab === 'edit') && (
        <>
          {/* Quick steps guide */}
          <div className="bg-gradient-to-br from-ink-card to-ink-el border border-bronze/15 rounded-card-lg px-4 py-3 mb-3.5">
            <div className="text-[10px] font-bold text-bronze tracking-wide mb-1">
